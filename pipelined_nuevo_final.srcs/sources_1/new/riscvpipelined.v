...
module riscvpipelined(
    ...
    wire        RegWriteD, ALUSrcD, MemWriteD, JumpD, JalrD, BranchED, BranchNED;
    wire [1:0]  ResultSrcD;
    wire [2:0]  ImmSrcD;
    ...
    assign InstrF   = InstrDecF; 
    
    rvc_decompressor decomp(
    .instr_in(Instr),
    .instr_out(InstrDecF),
    ...
    .Jump(JumpD), .Jalr(JalrD), .BranchE(BranchED), .BranchNE(BranchNED),
    ...
    .PCF(PCF),
    .InstrF(InstrDecF),
    .IsCompressedF(IsCompressedF),
    // ID
    .InstrD(InstrD),
    .Rs1D(Rs1D),   .Rs2D(Rs2D),
    .RegWriteD(RegWriteD), .ALUSrcD(ALUSrcD),
    .MemWriteD(MemWriteD), .JumpD(JumpD), .JalrD(JalrD),
    .BranchED(BranchED), .BranchNED(BranchNED),
